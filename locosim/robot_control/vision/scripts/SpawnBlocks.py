...
block_names = ['X1-Y1-Z2', 'X1-Y2-Z1', 'X1-Y2-Z2', 'X1-Y2-Z2-CHAMFER', 'X1-Y2-Z2-TWINFILLET', 'X1-Y3-Z2', 'X1-Y3-Z2-FILLET',
               'X1-Y4-Z2', 'X1-Y4-Z1', 'X2-Y2-Z2-FILLET']
blocks_info = {block_names[0]: (0.03, 0.03), block_names[1]: (0.03, 0.03), block_names[2]: (3, 6), block_names[3]: (0.03, 0.06),
               ...
               block_names[8]: (0.03, 0.12), block_names[9]: (0.06, 0.06)}
...
class Block:
    def __init__(self, name, position, orientation):
        ...
        span_x = np.round(L*sin(self.rotation) + l*cos(self.rotation), 5) + 2.
        span_y = np.round(L*cos(self.rotation) + l*sin(self.rotation), 5) + 2.
...
def spawnBlocks():
    ...
    for block in blocks_to_spawn:
        model_name = blocks_to_spawn[block]
        spawn_model_client(
            model_name=block,
            model_xml=open('/home/carro/ros_ws/src/locosim/ros_impedance_controller/worlds/models/'+str(model_name)+'/model.sdf', 'r').read(),
            robot_namespace='',
            initial_pose=Pose(position=Point(0, 0, 0.925), orientation=Quaternion(0, 0, 0, 0)),
...
def moveBlock(blocks):
    ...
    for block in blocks:
        ...
        a = random.uniform(0.0, pi)
        # a = pi / 4
        ms.pose.orientation.x = 0
        ms.pose.orientation.y = 0
        ms.pose.orientation.z = np.round(1 * sin(a), 5)
        ms.pose.orientation.w = np.round(1 * cos(a), 5)
