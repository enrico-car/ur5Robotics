...
class Listener:
    ...
    def callback(self, data, args):
        ...
        a,b,c=int_data
        # lista=[]
        # for i in range(int(xc_-30), int(xc_+30)):
        #     for j in range(int(yc_-30), int(yc_+30)):
        ...
        # self.y_tavolo=0.475-a         #0.475 dal worldframe del tavolo opertivo, 0.06 offset dall'optical left al centro della telecamera
        # self.x_tavolo=math.sqrt(depth**2-0.6**2-a**2)-0.4
        # print("x ", self.x_tavolo, "  y ", self.y_tavolo, self.cl)
    ...
    def tempMatch(self, xc, yc):
        ...
        img=cv2.imread(os.path.join(os.path.expanduser("~"),"ros_ws", "src", "locosim", "robot_control", "vision","scripts", "yolov5", "cv_img.jpg"), 0)
        gap=0
